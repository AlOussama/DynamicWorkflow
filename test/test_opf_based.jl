...
s_transformer= 2000; # transformer base power 2000MVA based on pypsa assumption
# sys_base = 100;
...
template= template_economic_dispatch(network=NetworkModel(ACPPowerModel;
# use_slacks=true,
))
...
plt = plot(title="Voltage Magnitude over Time", xlabel=names(v_m)[1])
